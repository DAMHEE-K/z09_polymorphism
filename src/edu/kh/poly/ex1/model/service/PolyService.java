...
public class PolyService {
	...
	public void ex1() {
		...
		// The method setBatteryCapacity(int) is undefined
		// for the type Car
		...
		// The method setDiscountOffer(double) is undefined for 
		// the type Car
		...
		//         -> Car의 상속 클래스는 모두 getEngine()을 가지고 있다를 정의
	...
	public void ex2() {
		...
		// arr[1]이 참조하는 객체가 Tesla면 true값 반환, 아니면 false
		System.out.println(arr[1] instanceof Tesla); // true
		
		// arr[1]이 참조하는 객체가 Spark면 true값 반환, 아니면 false
		System.out.println(arr[1] instanceof Spark); // false
		
		// arr[1]이 참조하는 객체가 Car면 true값 반환, 아니면 false
		...
		for(int i=0; i<arr.length; i++) {
			...
			// 부모 타입 검사는 제일 마지막에 진행해야 함
			if(arr[i] instanceof Tesla) {
				System.out.println("Telsa 객체 입니다.");
